...
**Built-in cards** ship in `letscode/builtin_agents/` (Explore, Plan, Review, SetupZed, DevHard, Worker, Tester), read via `importlib.resources`. A project `agents/<Name>.md` with the same stem overrides a built-in (case-insensitive). `--list-agents` shows all available cards with a `(built-in)` tag. No `--as` → no card → `build_system_prompt(model)` directly (no template layer).

### Agent Lifecycle Hooks (`hooks.py`)
A card may declare `onAgentStart` and/or `onAgentEnd` — paths to shell scripts the harness (`agent.py`) runs at the boundaries of a **big turn** (one complete `run_agent` invocation). `onAgentStart` runs once before the agent loop; its stdout is injected as a user message the LLM sees on its first turn. `onAgentEnd` runs once after the loop completes; its stdout is shown to the user. Return-code contract: **0** = stdout injected as user message; **2** = abort (onStart: skip agent loop entirely; onEnd: mark run as error); other non-zero = unexpected failure (warn). Both hooks receive a JSON summary on stdin (`{"turn": N, "tool_calls": [{"name", "success"}, ...]}`) and the `LETSCODE_STATE` env var (from `--state`). Scripts run in the project cwd under the card's sandbox preset; builtin card hooks resolve relative to the `builtin_agents/` package dir. If a declared script path doesn't exist, the hook is silently skipped. This enables non-invasive orchestration: a card's `onAgentEnd` script can chain to the next agent (`letscode --as Worker --state ...`), branch (test pass → stop, fail → retry), or parallelize (fork sub-agents) — all in plain shell, without harness-level control-flow logic.

### DevHard Orchestrator (`builtin_agents/DevHard.md`)
DevHard is a goal-driven orchestrator. Its agent loop **only runs Plan** (investigate + write `plan.md` with implementation steps and acceptance criteria). After the loop ends, the `onAgentEnd` hook (`hooks/devhard_loop.sh`) takes over as the **sole driver of the Worker-Tester cycle**: (1) spawns **Tester** to write test cases + `run_test.sh` (the acceptance contract), (2) spawns **Worker** to write implementation, (3) loops `verify.sh`→`run_test.sh`→exit-code: pass = done, fail = spawn Worker to fix → re-verify, up to 5 iterations then abort (exit 2). The LLM orchestrator does NOT spawn Worker or Tester — the hook does, deterministically. `verify.sh` (a fixed harness script) executes `run_test.sh` (produced by Tester per-task) and transparently returns its exit code, making the loop language/framework-agnostic. State is shared via `--state`. This fixes the core reliability problem: pass/fail judgment is a shell command's exit code, not an LLM's self-report.
